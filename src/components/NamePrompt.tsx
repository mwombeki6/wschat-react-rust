...
const NamePrompt = ({ vis, name, setName, setVis }: Props) => {
    ...
    return (
        <AnimatePresence>
            <motion.div className={vis ? 'z-40 transition-all flex flex-col justify-center items-center h-screen w-screen absolute backdrop-blur-xl' : "transition-all flex flex-col justify-center items-center h-screen w-screen absolute backdrop-blur-xl"}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}>
